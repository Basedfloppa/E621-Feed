...
//!
//! The actual physics simulation lives in [`super::graph_physics`]; this module
//! holds the Yew component, rendering, event handlers, and tag-business-logic
//! (backbone selection, PMI scoring, community detection).
...
/// Props for [`TagRelationGraphCard`].
...
/// Active pan drag (scene-level, not node-pin).
#[derive(Clone, Copy, Debug)]
struct PanState {
    active: bool,
    last_client_x: f64,
...
/// Wheel event listener handle kept alive for the effect's lifetime.
